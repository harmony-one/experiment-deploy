...
def parse_logs(database, table, keyword, pvar, s3_input, s3_output):
    ...
    with open('result.txt', 'w') as f:
        if results and len(results['ResultSet']['Rows']) > 1:
            ...
            f.write("%d match found for keyword %s on %s\n" % (len(results['ResultSet']['Rows'])-1, keyword, sys.argv[1]))
            for r in results['ResultSet']['Rows'][1:]:
                f.write("%s in log: %s\n" % (keyword, r['Data'][0]['VarCharValue']))
        else:
            f.write("no match found for keyword %s on %s\n" % (keyword, sys.argv[1]))
    return ret
