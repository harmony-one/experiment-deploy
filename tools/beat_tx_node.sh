...
[OPTIONS]
   ...
   -W wallet_url  the url to download wallet program (default: $WALLET_URL)
...
function do_download_wallet
{
   ...
   if [ "$CUSTOM_WALLET" == "true" ]; then
      curl -O ${WALLET_URL}/wallet
   else
...
while getopts "hp:vi:t:f:c:n:s:kw:p:W:" option; do
   case $option in
      h) usage ;;
      p) PROFILE=$OPTARG ;;
      v) VERBOSE=1 ;;
